...
namespace Email_OTP_Gen.Implementation
{
    public class EmailOtpService : IEmailOtpService
    {
        ...
        public int GenerateOtpEmail(string userEmail)
        {
            ...
            if (SendEmail(userEmail, emailBody))
            {
                ...
                return IEmailOtpService.STATUS_EMAIL_OK;
            }
            else
            {
                ...
                return IEmailOtpService.STATUS_EMAIL_FAIL;
            }
        }
        ...
        /// <returns></returns>
        public int CheckOtp(string userEmail, string otp)
        {
            // Checking is Email is Existed in the Sent Emails List
            if (!_userOtpData.ContainsKey(userEmail))
            {
                return IEmailOtpService.STATUS_OTP_FAIL; // OTP not generated yet
            }
            ...
            if (DateTime.Now.Subtract(timestamp).TotalSeconds > 60)
            {
                ...
                return IEmailOtpService.STATUS_OTP_TIMEOUT;
            }

            // Allow only 10 attempts to enter OTP
            if (attempts >= 10)
            {
                return IEmailOtpService.STATUS_OTP_FAIL;
            }
            ...
            if (otp == storedOtp)
            {
                ...
                return IEmailOtpService.STATUS_OTP_OK;
            }
            else
            {
